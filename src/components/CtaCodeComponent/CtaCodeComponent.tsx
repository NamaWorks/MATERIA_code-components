        ...
        <div className="relative overflow-hidden">
          <div className="flex">
            {title.split('').map((char, i) => (
              <span
                key={`top-${i}`}
                className={`char-top font-['Helvetica_Neue'] text-[12px] font-normal uppercase tracking-[0.02em] leading-none ${textColor} inline-block`}
              >
                {char === ' ' ? '\u00A0' : char}
              </span>
            ))}
          ...
          <div className="absolute inset-0 flex">
            {title.split('').map((char, i) => (
              <span
                key={`bottom-${i}`}
                className={`char-bottom font-['Helvetica_Neue'] text-[12px] font-normal uppercase tracking-[0.02em] leading-none ${textColor} inline-block`}
              >
                {char === ' ' ? '\u00A0' : char}
              </span>
            ))}
