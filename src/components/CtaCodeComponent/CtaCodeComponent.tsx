...
const ArrowIcon = ({ color }: { color: string }) => (
  <svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path
      ...
      stroke={color} strokeWidth="1.14286" strokeLinecap="round" strokeLinejoin="round"
    />
    <path
      className="cta-arrow-path"
      pathLength="1"
...
export const CtaCodeComponent = ({ title = 'Contacta ahora', variant = 'black' }: CtaCodeComponentProps) => {
  ...
  return (
    <>
      ...
      <div className="cta-wrapper group inline-flex items-center gap-[3px] cursor-pointer">
        <div className={`w-6 h-6 rounded-[1px] ${bg} flex items-center justify-center p-[2.29px] shrink-0 opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-[opacity,translate] duration-300 ease-in-out`}>
          <ArrowIcon color={iconColor} />
        </div>
        <div className={`rounded-[1px] ${bg} flex items-center justify-center px-2 py-[6px]`}>
          <span className={`font-['Helvetica_Neue'] text-[12px] font-normal uppercase tracking-[0.02em] leading-none ${textColor}`}>
            {title}
          </span>
        </div>
      </div>
